        ...
        network_manager.check_for_events(
            [&world_state](const std::vector<std::uint8_t> &data) {
                ...
                case Msg::ServerToClient::MsgId::NewWorldState: {
                    auto new_world_data = Msg::get_data_struct<
                        Msg::ServerToClient::NewWorldStateData>(
                        bytes_without_id);
                    world_state.cubes.clear();
                    for (const Msg::CubeData &cube_data :
                         new_world_data.cubes) {
                        world_state.cubes.push_back(Cube(
                            cube_data.pos_x, cube_data.pos_y, cube_data.pos_z,
